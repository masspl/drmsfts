...
class Admin_Controller extends MY_Controller {
  ...
  public $isAdmin = 0;
    ...
    public function getSchoolByAnchalCodeORDonor($anchalcode='',$fundingchapter='',$donorCode='')
    {
        $sql ="SELECT * FROM (SELECT * FROM mst_sif_details LEFT JOIN mst_donor ON md_code = msd_md_code) as schoolFund LEFT JOIN mst_funding_chapter ON mfc_id = schoolFund.msd_mfc_id WHERE msd_anchal_code LIKE '$anchalcode' AND msd_md_code ='$donorCode' AND msd_mfc_id ='{$fundingchapter}' ORDER BY msd_anchal_code";
        $schoollist=$this->db->query($sql)->result_array();
        return $schoollist;     
    }
}
